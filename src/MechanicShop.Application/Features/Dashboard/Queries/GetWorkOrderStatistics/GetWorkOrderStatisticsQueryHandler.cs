      ...
      var counts = await _context.WorkOrders
        ...
        var invoices = await _context.WorkOrders
                              ...
                              .Select(wo => new
                              {
                                  ...
                                  PartsCost  = wo.RepairTasks.SelectMany(rt => rt.Parts).Sum(p => p.Cost * p.Quantity),
                                  LaborCost  = wo.RepairTasks.Sum(rt => rt.LaborCost)
                              })
                              .ToListAsync(cancellationToken);

        var totalRevenue    = invoices.Sum(i => i.Total);
        var totalPartsCost  = invoices.Sum(i => i.PartsCost);
        var totalLaborCost  = invoices.Sum(i => i.LaborCost);
